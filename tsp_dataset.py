...
Adpated from https://github.com/Edward-Sun/DIFUSCO/blob/main/difusco/co_datasets/tsp_graph_dataset.py
"""
...
class TSPGraphDataset(torch.utils.data.Dataset):
    def __init__(self, data_file, sparse_factor=-1):
        ...
        logger.info(f'Loaded "{data_file}" with {len(self.file_lines)} lines')
    ...
    def __getitem__(self, idx):
        ...
        if self.sparse_factor <= 0:
            ...
            return (
                ...
                torch.from_numpy(points).float(),  # [n, 2]
                torch.from_numpy(adj_matrix).float(),  # [n, n]
                torch.from_numpy(tour).long(),  # [n + 1]
            )
        else:
            ...
            edge_index_0 = (
                torch.arange(points.shape[0]).reshape((-1, 1)).repeat(1, sparse_factor).reshape(-1)
            )
            ...
            graph_data = GraphData(
                x=torch.from_numpy(points).float(), edge_index=edge_index, edge_attr=tour_edges
            )
